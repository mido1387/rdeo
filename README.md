...
- Remove Duplicates: the optimized ensemble is passed to CREST `cregen`, which
...
8–12	200
≥ 13	300
Heuristic based on (https://doi.org/10.1021/ci2004658). Override with `-n`.

#### Requirements
- Python 3.9+ with RDKit (`pip install rdkit`)
- ORCA with the `XTB` and `EXTOPT` interfaces
- CREST for `cregen`
- For `gxtb`: an `xtb` build that accepts `--gxtb` (https://github.com/grimme-lab/g-xtb) 
- g-xTB in ORCA setup: (https://orcaforum.kofo.mpg.de/forum/index.php?thread/26658-how-to-use-the-new-g-xtb-in-orca/&postID=110453#post110453)
- For `uma` / `aimnet2`: the corresponding external-tool client executable, see (https://github.com/faccts/orca-external-tools) for tutorial
...
- `uma`	`! EXTOPT Opt` + `%method ProgExt ...`	Meta UMA MLIP; needs `uma_client` (most accurate/slowest in testing)
...
the machine. NOTE, due to g-xTB's current parallelization issues, it performs best with
a omp_threads_per_job = 1
...
OET server: Don't forget you need to activate an OET server to use UMA or AIMNet2 see (https://github.com/faccts/orca-external-tools) for more information
